...
class RoutePlanner(object):
    def __init__(self, pop_distance=8, max_distance=50):
        self.route = deque()
        # 如果node loc dis ===> dis < min 则去掉此点前的所有点
        self.min_dis = pop_distance # 最小考虑距离

        # 因为有些路会绕回来，避免把后面的给pop了 !!!!===> 重要!!
        self.max_distance = max_distance
        
    def set_route(self, goal_loc, gps=False):
        self.route.clear()
        for loc in goal_loc:
            self.route.append(loc)
        print(f"The whole path length is: {len(self.route)}")
    def run_step(self, now_loc):

        if len(self.route) == 1:
            print("======> only one route in list!!! <====")
            return self.route

        to_pop_end = 1
        cumulative_distance = 0.0

        for i, plan_loc in enumerate(self.route):

            if cumulative_distance > self.max_distance:
                break

            if i>0:
                cumulative_distance += plan_loc.distance(last_plan_loc)
                
            distance = plan_loc.distance(now_loc)
            if distance <= self.min_dis:
                to_pop_end = i
            last_plan_loc = plan_loc
            
        for _ in range(to_pop_end):
            if len(self.route) > 2:
                self.route.popleft()
...
class VehiclePIDController():
    ...
    def __init__(self, vehicle, args_lateral, args_longitudinal, offset=0, max_throttle=0.75, max_brake=0.8,
                 max_steering=0.8):
        """
        Constructor method.
